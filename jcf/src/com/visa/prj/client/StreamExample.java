...
public class StreamExample {
	...
	public static void main(String[] args) {
		...
		products.stream().filter(p->p.getCategoy().equals("mobile")).forEach(p->System.out.println(p));
		
		products.stream().filter(p->p.getCategoy().equals("mobile")).forEach(System.out::println);
		...
		products.stream()
		.filter(p->p.getCategoy().equals("mobile"))
		.map(p->p.getName())
		.forEach(name->System.out.println(name));
		
		
		List<Product> mobiles = products.stream().filter(p->p.getCategoy().equals("mobile")).collect(Collectors.toList());
